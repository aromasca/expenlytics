...
const RAW_EXTRACTION_PROMPTS: Record<ProviderName, PromptTemplate> = {
  anthropic: {
    user: `You are a precise financial document parser. First, identify the type of financial document, then extract ALL transactions.
...
const LEGACY_EXTRACTION_PROMPTS: Record<ProviderName, PromptTemplate> = {
  anthropic: {
...
export function getLegacyExtractionPrompt(provider: ProviderName): PromptTemplate {
  return LEGACY_EXTRACTION_PROMPTS[provider]
}
